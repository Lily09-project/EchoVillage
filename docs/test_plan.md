...
執行專案參數 `--visual-qa`，使用實際 GPU 產生十張 1280×720 PNG：主選單、設定、交易、開場、黎明、正午、夜晚、危險事件、任務進行與森林完成。人工檢查遮擋、對比、層級、文字截斷與畫面一致性。

## 發行驗證

`build_release.bat` 依序執行：66 項測試 → PCK 匯出 → portable runtime 組裝 → 120 幀成品煙霧測試。最後另檢查成品檔案、大小與殘留程序。
